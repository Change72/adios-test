...
//        Req.Data = srcData.get();
        ...
        std::vector<adios2::Remote::GetHandle> handles;
        ...
        if (getenv("DoRemote"))
        {
            ...
            {
                auto new_data = malloc(Nx * sizeof(T));
                Req.Data = new_data;
                auto handle = m_Remote->Get(Req.VarName, Req.RelStep, Req.BlockID, Req.Count, Req.Start, Req.Data);
                remote_data.push_back(new_data);
                handles.push_back(handle);
            }

            for (size_t i = 0; i < PendingGetRequests.size(); i++)
            {
                auto handle = handles[i];
                auto new_data = remote_data[i];
                if (m_Remote->WaitForGet(handle))
                {
                    std::cout << "remote get done" << std::endl;
                    std::cout << "remote data: ";
                    for (size_t j = 0; j < Nx; j++)
                    {
                        std::cout << ((T*)new_data)[j] << " ";
                    }
                    std::cout << std::endl;
                }
            }
